...
class Storage():
    ...
    def capacity(self, val):
        try:
            if int(val) < 0: raise PositiveValueError
    ...
    def recieve_equipment(self, obj, attr_name='_amount'):
        """
        ...
        if not isinstance(obj, (Printer, Scaner, Xerox)):
            raise ValueError("The second arg must be a subclass of OfficeEquipment")
        
        print("Receiving starts")
        temp_dict = dict(zip([attr for attr in obj.__dict__.keys()],[val for val in obj.__dict__.values()]))
        try:
            if self._capacity == Storage.max_capacity:
                raise StorageCapacityError
        except StorageCapacityError:
            print("The storage is full")
        else:
            if self._capacity + obj._amount < Storage.max_capacity:
                self.storage_items_dict[obj.__class__.__name__] = temp_dict
                self._capacity += obj._amount
                print(f"{obj._amount} {obj.name} (type {obj.__class__.__name__}) were added succesfully")
            else:
                temp_dict[attr_name] = Storage.max_capacity - self._capacity
                self.storage_items_dict[obj.__class__.__name__] = temp_dict
                print(f"Warning: only {Storage.max_capacity - self._capacity} {obj.name} (type {obj.__class__.__name__}) was added due to capacity limit.")
                self._capacity = Storage.max_capacity
    
    def give_equipment(self, model_name, dep_name, amount, attr_name='_amount'):
        ...
        try:
            for device_type,device_dict in self.storage_items_dict.items():
                ...
                if device_dict[attr_name] < amount:
                    ...
                    self._capacity -= device_dict[attr_name]
...
class OfficeEquipment:
    ...
    def __str__(self):
        return 'Type: {0.__class__.__name__}, model: {0.name}, price: {0._price}, amount: {0._amount}'.format(self)
